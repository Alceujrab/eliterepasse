<x-filament-panels::page>
        ...
        <div class="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="text-white">
                <div class="flex items-center gap-3 mb-1">
                    ...
                    <div>
                        <h1 class="text-2xl font-black tracking-tight">WhatsApp — Instâncias</h1>
                        <p class="text-green-200 text-sm">Evolution GO · {{ $this->instancias->count() }} instância(s) configurada(s)</p>
                    </div>
                </div>
            </div>
            <button wire:click="novaInstancia"
                class="flex items-center gap-2 bg-white text-[#075E54] font-black px-5 py-3 rounded-xl shadow-lg hover:bg-green-50 transition text-sm">
    ...
    @if($qrCodeBase64 !== null || $qrInstanciaNome)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl p-8 max-w-sm w-full text-center">
                ...
                <p class="text-sm text-gray-500 mb-5">{{ $qrInstanciaNome }}</p>
                ...
                @if($qrCodeBase64)
                    ...
                    <p class="text-xs text-red-400 font-semibold">⚠️ Expira em ~40 segundos</p>
                @else
                    <div class="py-8 text-gray-400">
                        <div class="text-4xl mb-3">⚠️</div>
                        <p class="text-sm">Não foi possível obter o QR Code.<br>A instância pode já estar conectada.</p>
                    </div>
                @endif
    ...
    @if($testInstanceId)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(0,0,0,0.6);">
            <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl p-6 max-w-sm w-full">
                ...
                <p class="text-xs text-gray-500 mb-4">Instância: {{ $this->instancias->find($testInstanceId)?->nome }}</p>
                ...
                <input wire:model="testPhone" type="text"
                    ...
                    class="w-full rounded-xl border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white px-4 py-3 text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent mb-4"/>

                <div class="flex gap-3">
                    <button wire:click="$set('testInstanceId', null)"
                        class="flex-1 py-3 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-xl font-semibold hover:bg-gray-200 transition text-sm">
                        Cancelar
                    </button>
                    <button wire:click="enviarTeste"
                        class="flex-1 py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-black transition text-sm flex items-center justify-center gap-2">
                        <span>Enviar</span>
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm mb-6 overflow-hidden">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {{-- Nome --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Nome da Instância *</label>
                        <input wire:model="nome" type="text" placeholder="Ex: Principal, Suporte, Vendas"
                            class="w-full rounded-xl border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-3 text-sm focus:ring-2 focus:ring-green-500 focus:border-transparent"/>
                        @error('nome') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    {{-- ID Evolution --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">ID da Instância (Evolution) *</label>
                        <input wire:model="instancia" type="text" placeholder="Ex: elite-repasse"
                            class="w-full rounded-xl border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-3 text-sm font-mono focus:ring-2 focus:ring-green-500 focus:border-transparent"/>
                        @error('instancia') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    {{-- URL Base --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">URL Base da API *</label>
                    ...
                    {{-- API Key --}}
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">API Key *</label>
                ...
                <div class="flex gap-6 mt-5">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">Instância Ativa</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input wire:model="padrao" type="checkbox" class="w-4 h-4 accent-green-500 rounded"/>
                        <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">⭐ Instância Padrão do Sistema</span>
                    </label>
                </div>
                ...
                <div class="flex justify-end gap-3 mt-6 pt-5 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <button wire:click="salvar"
                        class="px-6 py-2.5 bg-gradient-to-r from-[#128C7E] to-[#075E54] text-white rounded-xl font-black hover:opacity-90 transition text-sm shadow-lg shadow-green-500/20">
    ...
    @if($this->instancias->isEmpty())
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm py-20 text-center">
            ...
            <p class="text-gray-400 text-sm mb-6">Adicione sua instância Evolution para ativar o WhatsApp.</p>
            <button wire:click="novaInstancia"
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden hover:shadow-md transition group">
                    ...
                    <div class="px-5 pt-5 pb-4">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($inst->padrao)
                                            <span class="text-[10px] bg-yellow-100 text-yellow-700 font-black px-1.5 py-0.5 rounded-full">⭐ PADRÃO</span>
                                        @endif
                                    </div>
                                    <p class="text-xs text-gray-400 font-mono mt-0.5">{{ $inst->instancia }}</p>
                                </div>
                            </div>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl px-3 py-2 mb-4">
                            <p class="text-xs text-gray-500 dark:text-gray-400 truncate">🌐 {{ $inst->url_base }}</p>
                    ...
                    <div class="border-t border-gray-100 dark:border-gray-700 px-4 py-2.5 flex justify-end">
                        <button wire:click="excluir({{ $inst->id }})"
                            ...
                            class="text-xs text-red-400 hover:text-red-600 dark:hover:text-red-300 font-semibold transition flex items-center gap-1">
    ...
    <div class="mt-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-2xl p-5">
        <h4 class="text-sm font-black text-blue-800 dark:text-blue-200 mb-3">ℹ️ Como configurar uma instância Evolution GO</h4>
