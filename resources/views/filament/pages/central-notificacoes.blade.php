<x-filament-panels::page>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        @foreach($kpis as $kpi)
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm">
                ...
                <div class="text-2xl font-black text-gray-900 dark:text-white">{{ number_format($kpi['value']) }}</div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <div class="flex items-center gap-2">
            ...
            <select wire:model.live="filtroPeriodo"
                class="text-sm rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 px-3 py-1.5">
            ...
            @if($resumo['nao_lidas_total'] > 0)
                <button wire:click="marcarTodasLidas"
                    class="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-3 py-1.5 rounded-lg font-semibold hover:bg-green-200 transition">
                    ✓ Marcar todas lidas
                </button>
            @endif
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        ...
        <div class="px-5 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
            ...
            <span class="text-xs text-gray-400">Atualizando automaticamente</span>
        </div>
        ...
        <div class="divide-y divide-gray-50 dark:divide-gray-700">
            @forelse($notificacoes as $notif)
                ...
                <div class="flex items-start gap-4 px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition {{ $lida ? 'opacity-75' : '' }}">

                    {{-- Ícone --}}
                    <div class="flex-shrink-0 w-10 h-10 rounded-xl bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 shadow-sm flex items-center justify-center text-lg leading-none">
                        {{ $icone }}
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-0.5 flex-wrap">
                            <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">{{ $titulo }}</span>
                        ...
                        @if($mensagem)
                            <p class="text-xs text-gray-500 dark:text-gray-400 line-clamp-1">{{ $mensagem }}</p>
                        @endif
                    </div>
    ...
    <div class="mt-6 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
        <h3 class="text-sm font-bold text-gray-700 dark:text-gray-300 mb-4">📋 Notificações Automáticas do Sistema</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            @foreach($automaticas as $item)
                <div class="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg">
                    <span class="text-xl leading-none flex-shrink-0">{{ $item['icone'] }}</span>
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-gray-800 dark:text-gray-200">{{ $item['titulo'] }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ $item['desc'] }}</p>
                        <span class="inline-block mt-1 text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 font-bold px-2 py-0.5 rounded-full">
                            {{ $item['canais'] }}
                        </span>
                    </div>
                </div>
